////////////////////////////////////////////////////////////////////////////////////////////////////////
// Updated 9/8/2016 by p!                                                                      
// Attract-Mode "ZeroBOY" layout. Thanks to Robospin, on which this layout is based on                              
// When game marquee is turned off, console marquees will be turned on.     
////////////////////////////////////////////////////////////////////////////////////////////////////////   

class UserConfig {
   </ label="Select background art", help="Choice of blue, retro or flyer for background", options="black,flyer", order=1 /> enable_bg="black";   
   </ label="Select cab skin", help="Select a cab skin image", options="zeroboy", order=2 /> enable_cab="zeroboy";
   ...
   </ label="Select listbox, wheel, vert_wheel", help="Select wheel art type", options="listbox, wheel, vert_wheel", order=5 /> enable_list_type="wheel";
   ...
   </ label="Select pointer", help="Select animated pointer", options="hand,hand2,none", order=13 /> enable_pointer="hand"; 
   </ label="Enable text frame", help="Show text frame", options="yes,no", order=14 /> enable_frame="yes"; 
   </ label="Enble background Scanline", help="Show scanline effect", options="light,medium,dark", order=15 /> enable_scanline="light";
...
{
local b_art = fe.add_image("bkg.png", 0, 0, flw, flh );
b_art.alpha=255;
}

if ( my_config["enable_bg"] == "flyer") 
{
local surface_flyer = fe.add_surface( 1024, 768 );
local b_art = FadeArt( "flyer", 0, 0, 1024, 768, surface_flyer );
b_art.trigger = Transition.EndNavigation;
b_art.preserve_aspect_ratio = false;
b_art.alpha=127;

//now position and rotate surface for flyer
surface_flyer.set_pos(flx*0.42, fly*0.04, flw*0.38, flh*0.8);
surface_flyer.rotation = 3;
}
...
//cabinet image
if ( my_config["enable_cab"] == "zeroboy" )
{
 local cab = fe.add_image( "zeroboy.png", 0, 0, flw, flh );
}
...
{
local frame = fe.add_image( "frame.png", 0, fly*0.93, flw, flh*0.06 );
frame.alpha = 255;
}
...
{
local listbox = fe.add_listbox( flx*0.45, fly*0.06, flw*0.41, flh*0.9 );
listbox.rows = 15;
listbox.charsize = 24;
...
{
local point = fe.add_image("pointers/hand.png", flx*0.88, fly*0.34, flw*0.2, flh*0.35);
...
if ( my_config["enable_pointer"] == "hand2") 

 {
 local point = fe.add_image("pointers/hand2.png", flx*0.88, fly*0.34, flw*0.2, flh*0.35);
 local alpha_cfg = {
    when = Transition.ToNewSelection,
    property = "alpha",
    start = 110,
...
local glogo1 = fe.add_image("glogos/unknown1.png", flx*0.12, fly*0.935, flw*0.045, flh*0.05);
...
{
local mlogos = fe.add_image("mlogos/[Manufacturer]", flx*0.01, fly*0.935, flw*0.06, flh*0.05 );
